...
for(i in 1:10){ #-loop for group 2
  datnew2 <- dat[,2:7][dat[,1]=="2",] #-subset of data - group 1
  H.scv <- Hscv(datnew2)
  fhat2 <- kde(na.omit(datnew2[idxk!=i,]), H.scv, eval.points=na.omit(datnew2[idxk==i,])) #-kde - training as data, test as eval
  post.probk2[idxk==i]=predict(fhat2, x=na.omit(datnew2[idxk==i,])) #-populating group 1 matrix/predict using test data
}

post.probk3=matrix(rep(NA,1*90),ncol=1) #-create empty matrix for group 2

for(i in 1:10){ #-loop for group 3
  datnew3 <- dat[,2:7][dat[,1]=="3",] #-subset of data - group 1
  H.scv <- Hscv(datnew3)
  fhat3 <- kde(na.omit(datnew3[idxk!=i,]), H.scv, eval.points=na.omit(datnew3[idxk==i,])) #-kde - training as data, test as eval
  post.probk3[idxk==i]=predict(fhat3, x=na.omit(datnew3[idxk==i,])) #-populating group 1 matrix/predict using test data
}
