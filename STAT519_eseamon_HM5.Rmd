---
title: "University of Idaho Statistics 519 Spring 2015 Homework #5"
author: "Author: David Erich Seamon - erichs@uidaho.edu "
date: "Due Date: 05/01/2015"
header-includes:
- \usepackage{fancyhdr}
- \usepackage{enumitem}
...
- \fancyfoot[CO,CE]{Statistics 519 - Homework 5 - 05/01/2015}
...
\noindent {\bf ASSIGNED PROBLEM: Fit the five supervised classification models (LDQ, QDA, KDA, 9-nearest ngb, Tree) and two unsupervised clustering models (H. Clustering and kmeans) to the Football data (Table 8.3). For these 7 models, produce 20 m-CV error estimates (by using different m-fold partitions) for m=10 and m=2. }

\bfseries
\noindent
\bf

\noindent {\bf Solution - Step 1: Linear Discriminant Analysis with 10-fold CV - performed 20 times.}
...
misratelda10_20 <- matrix(NA, nrow=20, ncol=1)
...
misratelda10_20[j]=(90-sum(diag(table(gp,pred.gp))))/90 #misclassification rate

#prob=round(post.prob[rbind(1,31,61),], digits=2) #--1st, 31st, and 61st probabilities
}

#--2fold CV for LDA
misratelda2_20 <- matrix(NA, nrow=20, ncol=1)

for (j in 1:20) {
#set.seed(123)
idx=sample(1:2,90,rep=T)
#classify=function()
#{
dat <- read.table("/nethome/erichs/multivariate_analysis/T8_3_FOOTBALL.DAT")
...
for(i in 1:2) post.prob[idx==i,]=predict(lda(dat[idx!=i,-1], gp[idx!=i]), 
dat[idx==i,-1])$posterior #cv loop for lda
...
misratelda2_20[j]=(90-sum(diag(table(gp,pred.gp))))/90 #misclassification rate
...
Answer - Step 1: LDA results are located in the summarized table at the end of the document.
